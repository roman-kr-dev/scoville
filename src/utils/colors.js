...
function processValue(dataType, value) {
  switch (dataType) {
    case HEX:
      {
...
function createGradiant({ start, end, steps }) {
    var val1RGB = processValue(getType(start), start);
    var val2RGB = processValue(getType(end), end);
    var colors = [
